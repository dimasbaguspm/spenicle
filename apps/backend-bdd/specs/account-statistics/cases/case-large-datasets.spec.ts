...
test.describe("Account Statistics - Large Datasets and Performance", () => {
  ...
  test("budget health tracks multiple active and past budgets efficiently", async ({
    accountAPI,
    accountStatisticsAPI,
    budgetAPI,
  }) => {
    const accountRes = await accountAPI.createAccount({
      name: `perf-budget-multi-${Date.now()}`,
      note: "multi budget",
      type: "expense",
    });
    const accountId = accountRes.data!.id as number;
    ...
    const endDate = new Date(now.getTime() + 365 * 24 * 3600 * 1000);

    const budgetIds = [];

    // Create 10 past budgets
    for (let i = 0; i < 10; i++) {
      const monthOffset = 10 - i;
      const res = await budgetAPI.createBudget({
        accountId,
        name: `past-budget-${i}-${Date.now()}`,
        amountLimit: 5000 + i * 500,
        periodStart: new Date(
          now.getTime() - (monthOffset + 1) * 30 * 24 * 3600 * 1000,
        ).toISOString(),
        periodEnd: new Date(
          now.getTime() - monthOffset * 30 * 24 * 3600 * 1000,
        ).toISOString(),
      });
      if (res.data?.id) {
        budgetIds.push(res.data!.id as number);
      }
    }

    // Create 5 active/future budgets
    for (let i = 0; i < 5; i++) {
      const monthOffset = i;
      const res = await budgetAPI.createBudget({
        accountId,
        name: `future-budget-${i}-${Date.now()}`,
        amountLimit: 5000 + i * 500,
        periodStart: new Date(
          now.getTime() + monthOffset * 30 * 24 * 3600 * 1000,
        ).toISOString(),
